...
# ---------------------------------------------------------------------------
# 阵容质量 → Elo 修正权重（保守：阵容质量与 FIFA 排名高度相关，权重过大会重复
# 计强弱、拉大差距、把平局压没。0.5/20 下平局率回到 ~15%，仍保留阵容信号）
SQUAD_W_TOP5 = 0.5     # 五大联赛占比每 +1% → +0.5 Elo（0→100% 即 0→50）
SQUAD_W_AGE = 20       # 年龄曲线最大 ±20
